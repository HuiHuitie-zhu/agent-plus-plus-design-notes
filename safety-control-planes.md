...
不是因为代码多复杂（才 255 行），而是因为它是**架构级的安全，不是提示词级的安全**。
...
这个系统一共 255 行 Python，写了大概半天。它不完美，但它在项目存活期间没让 Agent 执行过一次危险操作。
...
*相关代码：`hooks/safety.py`（255 行）、`hooks/bash_parser.py`（484 行）、`hooks/bash_classifier.py`（430 行）*
